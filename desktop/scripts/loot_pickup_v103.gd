...
func _instantiate_authored_item() -> Node3D:
    if item_id == "badge":
        if cached_keycard == null and ResourceLoader.exists(KEYCARD_SCENE):
            ...
            if key_resource is PackedScene:
                cached_keycard = key_resource
...
static func release_cached_assets() -> void:
    if cached_pack_source != null and is_instance_valid(cached_pack_source):
        cached_pack_source.free()
    cached_pack_source = null
    cached_pack = null
    cached_keycard = null
