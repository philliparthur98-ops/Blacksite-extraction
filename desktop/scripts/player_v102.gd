...
# Raid-persistent weapon state + authored first-person presentation.
...
func _persist_current_weapon() -> void:
    if weapon_data.is_empty() or weapon_ids.is_empty(): return
    var id: String = weapon_ids[clampi(current_weapon_index,0,weapon_ids.size()-1)]
    weapon_runtime[id] = {"ammo":ammo,"reserve":reserve}
...
func _find_weapon_socket_bone(skeleton: Skeleton3D) -> String:
    for exact in ["socket.r","socket_r","weapon.r","weapon_r","hand.r","hand_r","RightHand","right_hand"]:
        if skeleton.find_bone(exact) >= 0: return exact
...
func _configure_fp_meshes(node: Node) -> void:
    ...
    for child in node.get_children(): _configure_fp_meshes(child)
...
func _build_first_person_arms() -> void:
    ...
    if ResourceLoader.exists(asset):
        ...
        if packed is PackedScene:
            arms_root = Node3D.new(); arms_root.name = "FirstPersonArms"; weapon_anchor.add_child(arms_root)
            authored_arms_model = (packed as PackedScene).instantiate()
            authored_arms_model.name = "Authored_FirstPersonRig"
            ...
            if authored_skeleton != null:
                ...
                if socket_bone != "":
                    ...
                    return
            arms_root.queue_free(); arms_root=null; authored_arms_model=null; authored_skeleton=null; weapon_socket=null
    super._build_first_person_arms()

func _apply_weapon_grip_transform() -> void:
    if viewmodel == null: return
    var id: String = weapon_ids[clampi(current_weapon_index,0,weapon_ids.size()-1)]
    # These are socket-local grip placements. The previous build preserved the
    # old camera-space transform during reparenting, which made the gun appear
    # to float even though it was technically parented to the hand.
    match id:
        "g17":
            viewmodel.position = Vector3(0.005,0.030,-0.075)
            viewmodel.rotation_degrees = Vector3(-2.0,180.0,-3.0)
        "m870":
            viewmodel.position = Vector3(-0.018,0.058,-0.185)
            viewmodel.rotation_degrees = Vector3(2.5,180.0,-5.0)
        _:
            viewmodel.position = Vector3(-0.014,0.054,-0.145)
            viewmodel.rotation_degrees = Vector3(2.0,180.0,-5.5)
    viewmodel.scale = Vector3.ONE

func _socket_current_viewmodel() -> void:
    if not uses_authored_fp_arms or weapon_socket == null or viewmodel == null: return
    if not is_instance_valid(weapon_socket) or not is_instance_valid(viewmodel): return
    if viewmodel.get_parent() != weapon_socket:
        # IMPORTANT: do not keep the prior camera-space transform. Snap into the
        # hand attachment and then apply a deliberate weapon-specific grip offset.
        viewmodel.reparent(weapon_socket,false)
    _apply_weapon_grip_transform()
...
func _load_weapon(index: int) -> void:
    ...
    if had_state:
        ...
        ammo = int(state.get("ammo",ammo)); reserve = int(state.get("reserve",reserve))
    else:
        weapon_runtime[target_id] = {"ammo":ammo,"reserve":reserve}
    _socket_current_viewmodel(); _emit_hud()

func _consume_round_state() -> bool:
    if dead or reloading or fire_cooldown > 0.0 or ammo <= 0: return false
    ammo -= 1
...
func _try_fire() -> void:
    if dead or reloading or fire_cooldown > 0.0 or Input.mouse_mode != Input.MOUSE_MODE_CAPTURED: return
    if ammo <= 0:
        toast_requested.emit("MAGAZINE EMPTY")
        return
    if not _consume_round_state(): return
    ...
    muzzle_mesh.visible = true; muzzle_mesh.rotation_degrees.z = randf_range(0.0,360.0)
    ...
    if shot_audio and shot_audio.stream:
        shot_audio.pitch_scale = randf_range(0.97,1.025); shot_audio.play()
    ...
    for i in range(pellets): _fire_ray(i,pellets,arm_ratio)
    _emit_hud()

func _finish_reload() -> void:
    super._finish_reload(); _persist_current_weapon()

func _process(delta: float) -> void:
    super._process(delta)
    if dead or not uses_authored_fp_arms or authored_arms_model == null or not is_instance_valid(authored_arms_model): return
    authored_motion_time += delta
    var target_pos := Vector3.ZERO
    var target_rot := Vector3.ZERO
    var idle_breath := sin(authored_motion_time*1.8)*0.006
    if sprinting:
        target_pos=Vector3(0.015,-0.030,0.018); target_rot=Vector3(-4.0,-2.0,4.5)
    elif reloading:
        var progress := 1.0-clampf(reload_timer/maxf(reload_total,0.01),0.0,1.0)
        target_pos=Vector3(-0.015,-0.018,0.015+sin(progress*PI)*0.018); target_rot=Vector3(3.0+sin(progress*PI)*4.0,2.5,-5.0)
    elif aiming:
        target_pos=Vector3(0.0,-0.006,-0.008); target_rot=Vector3.ZERO
    else:
        target_pos=Vector3(0.0,idle_breath,0.0); target_rot=Vector3(idle_breath*80.0,0.0,sin(authored_motion_time*1.15)*0.22)
...
func smoke_grip_alignment_ok() -> bool:
    if not has_authored_first_person_rig(): return false
    return viewmodel.position.length() < 0.28

func get_weapon_runtime_state(id: String) -> Dictionary:
    if id == weapon_ids[current_weapon_index]: _persist_current_weapon()
...
func smoke_ammo_persistence_regression() -> bool:
    ...
    if starting_mag < 4: return false
    fire_cooldown = 0.0
    if not _consume_round_state(): return false
    var after_fire_mag := ammo
    if after_fire_mag != starting_mag-1 or reserve != starting_reserve: return false
    _load_weapon(1); _load_weapon(0)
    if ammo != after_fire_mag or reserve != starting_reserve: return false
    fire_cooldown = 0.0
    if not _consume_round_state(): return false
    fire_cooldown = 0.0
    if not _consume_round_state(): return false
    ...
    if ammo != expected_mag or reserve != expected_reserve: return false
    _load_weapon(2); _load_weapon(0)
